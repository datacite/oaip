...
public class GetMetadata extends ServerVerb {
    ...
    if (baseURL == null) {
        ...
        if (debug) {
            log.info("GetMetadata.constructGetMetadata: identifier=" +
                               identifier);
            log.info("GetMetadata.constructGetMetadata: metadataPrefix="
                               + metadataPrefix);
        }
    ...
    try {
        ...
        || identifier == null || identifier.length() == 0) {
                if (debug) log.info("Bad argument");
        throw new BadArgumentException();
        }
        else if (!crosswalks.containsValue(metadataPrefix)) {
                if (debug) log.info("crosswalk not present: " + metadataPrefix);
        ...
        if (metadata != null) {
                    ...
                    if (debug) log.info("ID does not exist");
        ...
        if (debug) {
            log.info("GetMetadata.construct: contentType=" + crosswalks.getContentType(metadataPrefix));
            log.info("GetMetadata.construct: prerendered sb=" + sb.toString());
        }
